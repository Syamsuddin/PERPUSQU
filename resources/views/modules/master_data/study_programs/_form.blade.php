<div class="row g-3">
    <div class="col-md-6">
        ...
        <select class="form-select @error('faculty_id') is-invalid @enderror" name="faculty_id" required>
            <option value="">— Pilih Fakultas —</option>
            @foreach($faculties as $f)<option value="{{ $f->id }}" {{ old('faculty_id', $item->faculty_id ?? '') == $f->id ? 'selected' : '' }}>{{ $f->name }}</option>@endforeach
        </select>
